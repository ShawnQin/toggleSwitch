%********************************************************************
% PROGRAM NAME: negativeActive_LNA.m
% DESCRIPTION: 
...
for j1 = 1:N_k1;
    ...
    for j2 = 1:length(sigmae)
        ...
        variance_CLE_theory(j1) = C3(1,1)/steadyValue(j1,1)/OMIGA;   % CV shoud be used to characterize the variance ,revised on Jan 07,2016
        G3 = C3*expm(Ac');
        lagOneCLE_theory(j1) = G3(1,1)/C3(1,1);
        
        C4 = lyap(Acle,Dcle);
        corrcoef_ext_cle_theory(j1,j2) = C4(1,2)/(sqrt(C4(1,1)*C4(2,2)));
        variance_ext_cle_theory(j1,j2) = C4(1,1)/steadyValue(j1,1)/OMIGA;
        G4 = C4*expm(Acle');
        lagOneCLEext_theory(j1,j2) = G4(1,1)/C4(1,1);
